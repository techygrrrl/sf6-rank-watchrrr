...
async function perform() {
  ...
  const playerId = location.href.split('https://www.streetfighter.com/6/buckler/api/en/card/')[1]?.replace(/[^\d.-]+/g, '')
  ...
  const listResult = await databases.listDocuments(databaseId, collectionId, [])
  ...
  try {
    const recordResult = await databases[document ? 'updateDocument' : 'createDocument'](
      databaseId,
      collectionId,
      playerId,
      {
...
function getConfig() {
  ...
  if (!projectId) {
    throw new Error('user script misconfigured. missing: appwrite_project_id ')
  }
  ...
  if (!databaseId) {
    throw new Error('user script misconfigured. missing: appwrite_database_id ')
  }
  ...
  if (!collectionId) {
    throw new Error('user script misconfigured. missing: appwrite_collection_id ')
  }
